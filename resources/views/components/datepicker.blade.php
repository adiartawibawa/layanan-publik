...
@if ($type == 'single')
    <div x-data="app('{{ $default_date }}', '{{ strtoupper($format) }}')" x-init="[initDate(), getNoOfDays()]" x-cloak>
        <div class="relative w-full">
            ...
            <x-input {{-- class="bw-datepicker bw-input block w-full peer {{$name}}" --}} class="bw-datepicker" x-on:click="showDatepicker = !showDatepicker"
                x-model="datepickerValue" x-on:keydown.escape="showDatepicker = false" type="text"
                id="dtp-{{ $name }}" max_date="today" name="{{ $name }}" label="{{ $label }}"
                ...
                <div class="flex justify-between items-center mb-2">
                    <div>
                        <span x-text="MONTH_NAMES[month]"
                            class="text-lg font-bold text-gray-800 dark:text-gray-400 cursor-default"></span>
                ...
                <div class="flex flex-wrap mb-3 -mx-1">
                    <template x-for="(day, index) in DAYS" :key="index">
                        <div style="width: 14.26%" class="px-0.5">
                            <div x-text="day"
                                class="text-gray-800 dark:text-gray-400 font-medium text-center text-xs uppercase cursor-default">
                            </div>
                        </div>
                    </template>
                </div>
                    ...
                    <template x-for="(date, dateIndex) in no_of_days" :key="dateIndex">
                        <div style="width: 14.28%" class=" mb-1">
                            <div @click="getDateValue(date, '{{ $format }}')" x-text="date"
                                class="cursor-pointer text-center text-sm leading-8 rounded-full transition ease-in-out duration-100"
                                :class="{
                                    'bg-blue-200 dark:bg-slate-700': isToday(date) == true,
                                    'text-gray-600 dark:text-gray-100 hover:bg-blue-200 hover:dark:bg-slate-500': isToday(
                                        date) == false && isSelectedDate(date) == false,
                                    'bg-blue-500 dark:bg-slate-700 text-white hover:bg-opacity-75': isSelectedDate(
                                        date) == true
                                }">
                            </div>
                        </div>
...
@else
    <div class="grid grid-cols-2 gap-2">
        <div>
            <x-datepicker name="{{ $date_from_name }}" type="single" placeholder="{{ $date_from_label }}"
                default_date="{{ $default_date_from ?? '' }}" required="{{ $required }}"
                label="{{ $date_from_label }}" format="{{ $format }}" />
        </div>
        <div>
            <x-datepicker name="{{ $date_to_name }}" type="single" placeholder="{{ $date_to_label }}"
