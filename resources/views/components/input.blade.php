...
@php
    ...
    if (!$transparentPrefix) {
        $transparent_prefix = $transparentPrefix;
    }
    if (!$transparentSuffix) {
        $transparent_suffix = $transparentSuffix;
    }
    if (!$prefixIsIcon) {
        $prefix_is_icon = $prefixIsIcon;
    }
    if (!$suffixIsIcon) {
        $suffix_is_icon = $suffixIsIcon;
    }
    ...
    if ($errorHeading !== $error_heading) {
        $error_heading = $errorHeading;
    }
    if ($prefixIconType !== $prefix_icon_type) {
        $prefix_icon_type = $prefixIconType;
    }
    if ($suffixIconType !== $suffix_icon_type) {
        $suffix_icon_type = $suffixIconType;
    }
    if ($prefixIconCss !== $prefix_icon_css) {
        $prefix_icon_css = $prefixIconCss;
    }
    if ($suffixIconCss !== $suffix_icon_css) {
        $suffix_icon_css = $suffixIconCss;
    }
    //--------------------------------------------------------------------
    ...
    if ($type == 'password' && $viewable) {
        $suffix =
            '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 cursor-pointer show-pwd" onclick="togglePassword(\'' .
            $name .
            '\', \'show\')">
<path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 cursor-pointer hide-pwd hidden" onclick="togglePassword(\'' .
            $name .
            '\', \'hide\')"><path stroke-linecap="round" stroke-linejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" /></svg>';
        $suffix_is_icon = true;
    }
@endphp

<div class="relative w-full dv-{{ $name }} @if ($add_clearing) mb-3 @endif">
    <input {{ $attributes->merge(['class' => "bw-input peer $is_required $name $placeholder_color"]) }}
        type="{{ $type }}" id="{{ $name }}" name="{{ $name }}" value="{{ $selected_value }}"
        autocomplete="off" placeholder="{{ $placeholder_label }}{{ $required_symbol }}"
        @if ($numeric) onkeypress="return isNumberKey(event, {{ $with_dots }})" @endif
        @if ($error_message != '') data-error-message="{{ $error_message }}"
            data-error-inline="{{ $show_error_inline }}"
            data-error-heading="{{ $error_heading }}" @endif />
    ...
    @if (!empty($prefix))
        <div class="{{ $name }}-prefix prefix text-sm select-none pl-3.5 pr-2 z-20 text-blue-900/50 dark:text-slate-400 absolute left-0 inset-y-0 inline-flex items-center @if (!$transparent_prefix) bg-slate-100 border-2 border-slate-200 dark:border-slate-700 dark:bg-slate-900/50 dark:border-r-0 border-r-0 rounded-tl-md rounded-bl-md @endif"
            ...
            @if ($prefix_is_icon)
                <x-icon name='{!! $prefix !!}' type="{{ $prefix_icon_type }}"
                    class="{{ $prefix_icon_css }}" />
            @else
                {!! $prefix !!}
            @endif
        </div>
        <script>
            positionPrefix('{{ $name }}', 'blur', '{{ $transparent_prefix }}');
        </script>
    @endif
    @if (!empty($suffix))
        <div class="{{ $name }}-suffix suffix text-sm select-none pl-3.5 pr-2 z-20 text-blue-900/50 dark:text-slate-400 absolute right-0 inset-y-0 inline-flex items-center @if (!$transparent_suffix) bg-slate-100 border-2 border-slate-200 border-l-0 dark:border-slate-700 dark:bg-slate-900/50 dark:border-l-0 rounded-tr-md rounded-br-md @endif"
            data-transparency="{{ $transparent_prefix }}">
            @if ($suffix_is_icon)
                <x-icon name='{!! $suffix !!}' type="{{ $suffix_icon_type }}"
                    class="{{ $suffix_icon_css }}" />
            @else
                {!! $suffix !!}
            @endif
        </div>
